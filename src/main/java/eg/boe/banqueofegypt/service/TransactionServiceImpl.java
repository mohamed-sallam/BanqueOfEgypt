...
public class TransactionServiceImpl implements TransactionService {
    ...
    @Override
    public Iterator<TransactionRetrievalDto> getAllTransactions() {
        return transactionRepository.findAll().stream()
                .map(it -> modelMapper.map(it, TransactionRetrievalDto.class))
    ...
    public TransactionRetrievalDto transact(TransactionPreservationDto transactionPreservationDto) {
        if (transactionPreservationDto.getAmount().isEmpty()) throw new BusinessException(400, "Amount is required");
        ...
        if (amount <= 0) throw new BusinessException(400, "Amount must be positive");
        if (amount <= 1000) throw new BusinessException(400, "Amount must be more than 1000");
        if (amount >= 100_000_000) throw new BusinessException(400, "Amount must be less than 100 Million");
        ...
        try {
            balanceResponse = clientService.getBalance(transaction.getPayer().getUrl());
        ...
        stack.push(clientService.withdraw(new WithdrawMoneyRequest(transaction.getAmount()), transaction.getPayer().getUrl()));
